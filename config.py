...
from networks.chinese_chess_network import ChineseChessNetwork
...
def make_chinese_chess_config() -> MuZeroConfig:
    def visit_softmax_temperature(num_moves, training_steps):
        """
        if num_moves < 30:
            return 1.0
    ...
    return MuZeroConfig(
        ...
        nb_epochs=20,
        network_args={
            'observatin_size': [6,10,9],
            'action_size': 90 * 90,
            'num_channel': 16,
            },
        network=ChineseChessNetwork,
        action_space_size=90 * 90,
        max_moves=512,
        discount=0.99,
        dirichlet_alpha=0.25,
        num_simulations=49,  # Odd number perform better in eval mode
        batch_size=512,
        td_steps=10,
        visit_softmax_temperature_fn=visit_softmax_temperature,
        lr=0.1)
